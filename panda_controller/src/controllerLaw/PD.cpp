...
namespace panda_controller
{
    ...
    void PD::dynamicSetParameter(panda_controller::panda_controller_paramConfig &config)
    {
        if (taskSpace == jointSpace)
        {
    ...
    void PD::pubData(panda_controller::paramForDebug &param_debug, my_robot::Robot<DIM> *robot)
    {
        for (int i = 0; i < DIM; i++)
        {
