...
int main(int argc, char **argv)
{
    ...
    for (JointIndex joint_id = 0; joint_id < (JointIndex)model.njoints; ++joint_id)
    {
        ...
        std::cout << J1 * v << std::endl
                  << std::endl; // the matrix J1 stored the results of the Jacobian; J1*v means the joint's velocity (relative to the world) expressed in the local joint frame.
        ...
        std::cout << J2 * v << std::endl
                  << std::endl; // the matrix J2 stored the results of the Jacobian;
    ...
    // *Perform the Inverse dynamics based on the recursive Newton - Euler Algorithm(RNEA) * *M(q) * d2q / dt2 + C(q, dq / dt) * dq / dt / +G(q) = tau + sum(J ^ T(q) * fext) *
    ...
    std::cout << data.tau << std::endl
              << std::endl; // the tau store the torque (expressed in the local coordinate revolute axis) that should be loaded on the joints to keep the state without external force/torque.
    for (JointIndex joint_id = 0; joint_id < (JointIndex)model.njoints; ++joint_id)
    {
        std::cout << std::setw(24) << std::left
                  << model.names[joint_id] << ": " << std::endl
    ...
    *it = F4;
    ...
    std::cout << data.tau << std::endl
              << std::endl; // the tau store the torque (expressed in the local coordinate revolute axis) that should be loaded on the joints to keep the state with the external force/torque.
    for (JointIndex joint_id = 0; joint_id < (JointIndex)model.njoints; ++joint_id)
    {
        std::cout << std::setw(24) << std::left
                  << model.names[joint_id] << ": " << std::endl
