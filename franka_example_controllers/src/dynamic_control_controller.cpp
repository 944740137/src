...
{
  std::cout << "--------------init1:JointDynamicControlController_4.28--------------" << std::endl;
  std::cout << "--------------init2:JointDynamicControlController_4.28--------------" << std::endl;
  // 参数服务器
  std::string arm_id;
  ...
  {
    ROS_ERROR("JointDynamicControlController: Invalid or no joint_names parameters provided, ""aborting controller init!");
    return false;
  }
...
{
  std::cout << "--------------start1:JointDynamicControlController_4.28--------------"<< std::endl;
  std::cout << "--------------start2:JointDynamicControlController_4.28--------------" << std::endl;
...
bool CartesianDynamicControlController::init(hardware_interface::RobotHW* robot_hw,ros::NodeHandle& node_handle) 
{
  ...
  if (!node_handle.getParam("joint_names", joint_names) || joint_names.size() != 7) 
  {
    ROS_ERROR("CartesianDynamicControlController: Invalid or no joint_names parameters provided, ""aborting controller init!");
    return false;
  }
